...
def list_files(path):
    """Round up the files for the season."""
...
def duplicate_round_guids(new_df_guids, existing_round_guids, matches):
    """Detect rounds that repeat across different files."""
    duplicate_rounds = []
    for guid in new_df_guids:  # for each guid in incoming df
        if guid in existing_round_guids:
            ...
            print(f"[!] Dup {guid} " + date + " " + size + " " + file)
...
tis_season = r"na\gsix\2021\2021May"
keep_only_pattern =""
#keep_only_pattern = "seasons_data\\2021"
all_seasons = "all"
