...
from rtcwlog.utils.ospstrings import process_OSP_line
...
from rtcwlog.utils.ospstrings import get_round_guid_osp
...
#        pivoted_weapons = pivoted_weapons[pivoted_weapons.columns[-2:]]
    ...
    def determine_current_map(self, currentRound):
        ...
        if(len(currentRound.map_counter) == 0):
            ...
            currentRound.map_name = tmp_map.name
        return tmp_map
    ...
    def build_matchdf(self, currentRound):
        ...
        new_match_line = MatchLine(self.file_date,                 # date of file creation
                                   ...
                                   currentRound.map_name)          # map - processed at the end
        return new_match_line
    ...
    def process_log_worker(self):
        ...
        self.map_class.load_maps(self.map_class) # TODO figure out how to pass nothing
        announcements = self.map_class.transpose_by_obj(self.map_class)
        ...
        for i, val in enumerate(self.lines):
            ...
            for key, value in log_lines.items():
                ...
                if x:                    
                    ...
                    if value.event == Const.EVENT_START:
                        ...
                        if currentRound.game_paused:
                            ...
                            if currentRound.game_started: #round aborted or otherwise interrupted
                                currentRound = Round(round_order) #start anew
                    ...
                    if currentRound.game_started and value.event == Const.EVENT_OSP_STATS_ACCURACY: #happens at the end of every round for a player that is ON A TEAM
                        round_num = int(line.split("(")[-1].split(" ")[0])
                        currentRound.round_num= round_num
                        break
                    
                    #^7TEAM   Player          Kll Dth Sui TK Eff ^3Gib^7    ^2DG    ^1DR   ^6TD  ^3Score
                    if currentRound.game_started and value.event == Const.EVENT_OSP_STATS_START and currentRound.reading_osp_stats == False:
                    ...
                    if value.event == Const.EVENT_OSP_STATS_ALLIES or value.event == Const.EVENT_OSP_STATS_AXIS: #OSP team stats per player
                        if currentRound.game_started:
                            ...
                            currentRound.osp_line_processed = True
                        break
                    ...
                    if currentRound.game_started and value.event == Const.EVENT_OSP_TIME_SET: #osp round 1 end.
                        ...
                        #TEMP_STORAGE
                        #tmp_r1_time = currentRound.round_time
                    ...
                    if value.event == Const.EVENT_OSP_REACHED or value.event == Const.EVENT_OSP_NOT_REACHED or value.event == Const.EVENT_OSP_TIME_SET: #had and game_finished
                        ...
                        currentRound.tmp_stats_all["round_order"] = round_order
                        ...
                        currentRound = self.determine_round_result(currentRound, value.event)
                        ...
                        currentRound.round_datetime = self.select_time(osp_demo_date, osp_demo_time, osp_map_date, osp_map_time, osp_stats_date, osp_stats_time, osp_jpeg_date, osp_jpeg_time, log_date)
                        ...
                        new_match_line = self.build_matchdf(currentRound)
                        self.matchesdf = self.matchesdf.append([vars(new_match_line)]) 
                        ...
                        print("[ ] Proccessed round " + str(currentRound.round_order).ljust(2) + " winner " + currentRound.winner.ljust(6) + " on " + currentRound.map_name[0:10].ljust(11) + ". Events: " + str(len(tmp_logdf)).ljust(6) + ". Players: " + str(len(currentRound.tmp_stats_all)).ljust(2) + "(Linestime: " + str(round((round_end_time - round_start_time),2)) + " s " + "Wraptime: " + str(round((wrap_end_time - wrap_start_time),2)) + " s)")
        ...
        if self.matchesdf.empty or self.statsdf.empty or self.logdf.empty:
            ...
            return {"logs":self.logdf, "stats":self.statsdf, "matches":self.matchesdf, "renames" : renameDF}