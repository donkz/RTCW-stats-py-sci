...
class Awards:
    ...
    def __init__(self, result, amendments = None, minimum_rounds = 40):
        ...
        self.minrounds = min(minimum_rounds,int(minrounds)) #20% or 40 rounds, which ever is less
    ...
    def fill_na_values(self):
        ...
        awardsdf['Kills'] = awardsdf['Kills'].fillna(0).astype(int)
        awardsdf['KPM'] = awardsdf['KPM'].fillna(0)
        awardsdf['KDR'] = awardsdf['KDR'].fillna(0)
        awardsdf['KillStreak'] = awardsdf['KillStreak'].fillna(0).astype(int)
    ...
    def award_megakill(self,event_lines_dataframe):
        ...
        if len(tmp_result) > 0: 
            ...
            megakills = megakills.merge(time_and_map, on='round_guid', how='inner', suffixes=('_events', '_matches'))
        ...
        if len(megakills) == 0: 
            return [result, None]
        return [result, megakills[["match_date","map", "killer", "mod", "victim","count"]]]
    ...
    def award_most_wins(self,sum_lines_dataframe):
        temp = sum_lines_dataframe[sum_lines_dataframe.game_result.isin(["WON","FULLHOLD"])][["Killer","game_result"]].reset_index()
        temp.loc[temp["game_result"] == "WON", "WinPoints"] = 2
        temp.loc[temp["game_result"] == "FULLHOLD", "WinPoints"] = 1
        result = temp.groupby(Const.STAT_BASE_KILLER)[["WinPoints"]].sum()
        result["WinPoints"] = result["WinPoints"].div(2)
        
        result = result.join(self.rounds, how='outer')
        
        result["full_games"] = result["Rounds"].div(2).apply(np.ceil)
        result["Win%"] = result["WinPoints"]/result["full_games"]
        ...
        result.drop(["WinPoints","Rounds", "full_games"], axis=1, inplace=True)
    ...
    def award_tapout(self,sum_lines_dataframe):
        ...
        resultdf["Tapout"] = resultdf["Tapout"].round(2).multiply(100).fillna(0).astype(int).apply(lambda x: '{0:0>2}'.format(x))+"%"
